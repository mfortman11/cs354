/* 
 * Assignment 1 Part 4 game2.c 
 * Written by Eric Eichstadt section 2 & Mike Fortman section 2
 * CS 354
 * 2/9/2015
 *
 * This program reads in an ascii file from a command line arguement
...
/* main uses 1 command line arguement to find and read an ascii file
...
/* createlist takes in a file pointer and creates a NULL Node pointer to
 * then reads through the file associated w/ the file pointer. Calls listadd
 ...
 * 
 * Parameters:
 *   fp: a pointer to a file to be read in
 */
Node *createlist(FILE * fp){
  char fileinput[11]; /*char array for buffer */
  Node *head = NULL; /*intializes a pointer to a head Node as NULL*/
  int num;
  while(fgets(fileinput, 11, fp)){
    num = atoi(fileinput); /*convers ascii input to an integer */
